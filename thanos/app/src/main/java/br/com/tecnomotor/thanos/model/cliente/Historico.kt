...
class Historico() {
    ...
    fun toHistoricoEntity():HistoricoEntity {
        return ConvertClass.convert(this, HistoricoEntity::class.java.name) as HistoricoEntity
    }

    override fun toString(): String {
        return "Historico(id=$id, montadoraId=$montadoraId, montadoraNome='$montadoraNome', veiculoId=$veiculoId, veiculoNome='$veiculoNome', motorizacaoId=$motorizacaoId, motorizacaoNome='$motorizacaoNome', tipoSistemaId=$tipoSistemaId, tipoSistemaNome='$tipoSistemaNome', sistemaId=$sistemaId, sistemaNome='$sistemaNome', conectorId=$conectorId, conectorNome='$conectorNome', posicaoConector='$posicaoConector', pinoX='$pinoX', pinoY='$pinoY', aplicacaoId=$aplicacaoId, modulo=$modulo, anoInicial=$anoInicial, anoFinal=$anoFinal, imgConVeiculo=$imgConVeiculo, data=$data)"
    }
}

fun HistoricoEntity.toHistorico(): Historico {
    return ConvertClass.convert(this, Historico::class.java.name) as Historico
}

fun Selecao.loadHistorico(historico: Historico) {
    this.setMontadora(historico.getMontadora())
        .setVeiculo(historico.getVeiculo())
        .setMotorizacao(historico.getMotorizacao())
        .setTipoSistema(historico.getTipoDeSistema())
        .setSistema(historico.getSistema())
        .setConector(historico.getConector())
        .setAplicacao(historico.getAplicacao())
